...
(function () {
  ...
  const CLUBS = [
    ...
    },
    {
      ...
      examples: 'Football, Basketball, Volleyball, Badminton, Chess, Table Tennis, Athletics',
    ...
    },
    {
      ...
      examples: 'Programming, Robotics, AI, Research, Hackathons, Electronics, Web Development',
      field: 'stem_interest'
    },
    {
      id: 'arts',
      ...
      examples: 'Painting, Sketching, Digital Art, Photography, Crafts, Decoration',
      field: 'arts_interest'
    },
    {
      id: 'social',
      ...
      examples: 'Community Service, Leadership, Campaigns, Blood Donation, Environment',
    ...
    },
    {
      ...
      examples: 'Singing, Dancing, Acting, Anchoring, Comedy, Beatboxing, Music, DJ',
      field: 'entertainment_interest'
    }
  ];

  const RATING_LABELS = ['', 'Curious', 'Interested', 'Keen', 'Excited', 'All in 🔥'];
  ...
  function buildDynamicQuestions() {
    ...
    dynamicQuestions.innerHTML = selected.map((c, i) => `
  ...
  function validatePhone(v) {
    if (!/^[9]\d{9}$/.test(v)) return 'Enter a valid Nepali number (10 digits, starts with 9).';
    return '';
  }
  ...
  function validateRoll(v) {
    ...
    if (isNaN(n) || n < 1 || n > 910) return 'Roll number must be between 1 and 910.';
    return '';
  }
  ...
  function validateStep(step) {
    ...
    if (step === 1) {
      ...
      checks.forEach(([el, msg]) => {
        if (msg) { setError(el, msg); ok = false; }
        else clearError(el);
      });
    }
  ...
  function bindValidation() {
    ...
    social.addEventListener('input', () => {
      social.value.trim() ? clearError(social) : setError(social, 'This field is required.');
    });
  }
  ...
  function populateSections(grade) {
    ...
    if (!grade) {
      ...
      section.innerHTML = '<option value="" disabled selected>Select grade first</option>';
      return;
    }
  ...
  async function handleSubmit(e) {
    ...
    CLUBS.forEach(c => {
      const ta = document.querySelector(`#dynamicQuestions textarea[name="${c.field}"]`);
  ...
  function launchConfetti() {
    ...
    const colors = ['#2563EB', '#60a5fa', '#a855f7', '#fbbf24', '#34d399', '#f472b6'];
    const pieces = [];
    const N = 140;
